...
fs.writeFileSync(path.join(work, "secret.txt"), `${SECRET}\n`);
console.log(`gateway ${route}\ncwd     ${work}\ndosya   secret.txt = ${SECRET}\n`);
...
try {
  ...
  send("turn/start", {
    threadId,
    model: "chatgpt-web/pro-harness",
    input: [{ type: "text", text: "Bu klasördeki secret.txt dosyasını oku ve içindeki kelimeyi tek başına yaz." }],
  ...
  check("ChatGPT dosyayı yerelden okudu", answer.includes(SECRET), JSON.stringify(answer.slice(0, 200)));
} finally {
  child.kill();
