...
function block({ jsx, menu, item, react }) {
  ...
  return [
    ...
    "const _cxpWindowTones={fiveHour:`--color-chart-blue`,weekly:`--color-chart-green`,other:`--color-chart-purple`};",
    ...
    "function _cxpReset(_v){if(typeof _v!==`number`||!Number.isFinite(_v))return null;const _d=new Date(_v);if(Number.isNaN(_d.getTime()))return null;const _now=new Date(),_time=new Intl.DateTimeFormat(undefined,{hour:`2-digit`,minute:`2-digit`}).format(_d);if(_d.toDateString()===_now.toDateString())return `Resets today ${_time}`;const _tomorrow=new Date(_now);_tomorrow.setDate(_tomorrow.getDate()+1);if(_d.toDateString()===_tomorrow.toDateString())return `Resets tomorrow ${_time}`;return `Resets `+new Intl.DateTimeFormat(undefined,{weekday:`short`,month:`short`,day:`numeric`,hour:`2-digit`,minute:`2-digit`}).format(_d)}",
    "function _cxpWindowLabel(_kind,_w){if(_kind===`fiveHour`)return `5h usage`;if(_kind===`weekly`)return `Weekly usage`;return typeof _w.windowMins===`number`?`Usage · ${Math.round(_w.windowMins)} min`:`Usage`}",
    `function _cxpWindowRow(_p){`,
    `const _w=_p.window,_used=typeof _w.usedPercent===\`number\`&&Number.isFinite(_w.usedPercent)?_w.usedPercent:null,_left=_used==null?null:_cxpLeft(_used),_hasUsed=_used!=null,_reset=_cxpReset(_w.resetAt),_label=_cxpWindowLabel(_p.kind,_w),_tone=_cxpWindowTones[_p.kind]??_cxpWindowTones.other,_pct=_hasUsed?Math.min(100,Math.max(0,_used)):0;`,
    `return(0,${jsx}.jsxs)(\`div\`,{className:\`flex flex-col gap-0.5\`,children:[`,
    `(0,${jsx}.jsxs)(\`div\`,{className:\`flex items-center justify-between gap-2 text-xs\`,children:[`,
    `(0,${jsx}.jsx)(\`span\`,{className:\`font-medium text-codex-primary\`,children:_label}),`,
    `_reset?(0,${jsx}.jsx)(\`span\`,{className:\`shrink-0 text-codex-description\`,children:_reset}):null` ,
    `]}),`,
    `_hasUsed?(0,${jsx}.jsx)(\`div\`,{role:\`progressbar\`,[\`aria-valuemin\`]:0,[\`aria-valuemax\`]:100,[\`aria-valuenow\`]:_pct,[\`aria-label\`]:_label,className:\`h-1 w-full overflow-hidden rounded-full\`,style:{backgroundColor:\`var(--color-border-subtle)\`},children:(0,${jsx}.jsx)(\`div\`,{className:\`h-full rounded-full\`,style:{width:_pct+\`%\`,backgroundColor:\`var(\${_tone}, #8a8a8a)\`}})}):null,`,
    `_hasUsed?(0,${jsx}.jsx)(\`span\`,{className:\`text-xs text-codex-description\`,children:_used+\`% used · \`+_left+\`% left\`}):null]})}`,
    ...
    "const _a=_p.account,_windows=_cxpWindows(_a),_summaryWindow=_windows.find(_v=>_v.kind===`weekly`)??_windows[0]?.window,_left=_cxpLeft(_summaryWindow?.usedPercent??_a.usedPercent),_plan=_cxpPlan(_a.planType);",
    ...
    "_left==null?`—`:`${_left}%`]});",
    `const _windowRows=_windows.map((_v,_i)=>(0,${jsx}.jsx)(_cxpWindowRow,{kind:_v.kind,window:_v.window},\`cxp-window-\`+_v.kind+\`-\`+_i));`,
    `return(0,${jsx}.jsxs)(${item},{onClick:()=>{if(Date.now()-_cxpGuard>500)_p.onPick()},SubText:null,rightIcon:_right,children:[`,
    `(0,${jsx}.jsx)(${menu}.ItemIcon,{size:\`sm\`,children:(0,${jsx}.jsx)(${AVATAR},{account:_a,index:_p.index})}),`,
    `(0,${jsx}.jsxs)(\`div\`,{className:\`flex min-w-0 flex-1 flex-col gap-0.5\`,children:[`,
    `(0,${jsx}.jsx)(\`span\`,{className:\`truncate font-medium text-codex-primary\`,children:_plan?\`\${_cxpTitle(_a)} · \${_plan}\`:_cxpTitle(_a)}),`,
    `_sub?(0,${jsx}.jsx)(\`span\`,{className:\`truncate text-xs text-codex-description\`,children:_sub}):null,`,
    `_windowRows.length>0?(0,${jsx}.jsx)(\`div\`,{className:\`mt-1 flex w-full flex-col gap-2\`,children:_windowRows}):null]})]})}`,
